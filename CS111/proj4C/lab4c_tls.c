...
void exec_cmd(char *cmd) {
  ...
  if (strcmp(cmd, "STOP") == 0) {
    ...
    SSL_write(ssl, output_str, strlen(output_str));
    run_flag = 0;
    raise(SIGINT);
  } else {
    char eq = '=';
...
void *read_cmds(void* nvrmnd) {
  (void)nvrmnd;   // not used for anything relevant
  while(run_flag) {
...
int main (int argc, char *argv[]) {
  ...
  cypher = SSL_CTX_new(client_method);
  if (cypher == NULL) {
    fprintf(stderr, "ERROR: Unable to initialize CTX function.\n");
    exit(1);
  }  
  ...
  sock_addr.sin_addr.s_addr = inet_addr(ip_addrr);
  if (connect(server, (struct sockaddr *) &sock_addr,
       sizeof(sock_addr)) != 0) {
    close(server);
  ...
  if (SSL_connect(ssl) < 0) {
    fprintf(stderr, "ERROR while connecting to TLS server.\n");
    exit(1);
  }
  ...
  hello_server[12] = '\n';
  if (SSL_write(ssl, hello_server, 13) < 0) {
  ...
  pthread_create(&read_thread, NULL, &read_cmds, NULL);
  while (run_flag) {
    if (!stop && run_flag) {
      time(&curr_time_raw);
      curr_time = localtime(&curr_time_raw);
	...
	temperature = temperature*9.0/5 + 32;
      ...
      if (SSL_write(ssl, output_str, strlen(output_str)) < 0) {
	fprintf(stderr, "ERROR while writting to server.\n");
	exit(1);
      } 
