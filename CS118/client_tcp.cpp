...
size_t buffered_sizes[BUFFER_SIZE];
int spot_taken[BUFFER_SIZE];
struct packet_hdr buffered_hdr[BUFFER_SIZE];
...
void move_window() {
	...
	for (int j = 0; j < BUFFER_SIZE; j++) {
		...
		int index = (buffered_hdr[j].seq_n - init_server_seqnum) / MAX_PACKET_SIZE;
		pwrite(filefd, BUFFER[j], buffered_sizes[j], index * (MAX_PACKET_SIZE - sizeof(struct packet_hdr)));
	    }	
	}
    }
...
void fin(uint64_t final_seqnum) {
	...
	if (pkt.ack_flag == 1 && pkt.fin_flag == 0) //Got the last FIN
	    exit(0);
...
void get_file() {
	...
	if (pkt.seq_n < rcv_base - BASE_RCV_WINDOW)
	    continue;
	if (pkt.seq_n < init_server_seqnum) //ignore handshake packets
	    continue;
	...
	if (pkt.seq_n < rcv_base) { //must send ACK to move sender window
	    pkt.ack_flag = 1;
	    pkt.fin_flag = 0;
	    pkt.ack_n = rcv_base;
	    send_hdr(&pkt);
	    ...
	    int index = (pkt.seq_n - init_server_seqnum) / MAX_PACKET_SIZE;
	    pwrite(filefd, data, bytes, index * (MAX_PACKET_SIZE - sizeof(struct packet_hdr)));
	    rcv_base = rcv_base + bytes + sizeof(struct packet_hdr);
	    move_window();
	    pkt.ack_flag = 1;
	    pkt.fin_flag = 0;
	    pkt.ack_n = rcv_base;
...
void *first_packet(void *arg) { //this thread handles the first data packet received
    filefd = open("transmit.txt", O_CREAT | O_TRUNC | O_WRONLY, S_IRUSR | S_IWUSR);
	...
	if (pkt.seq_n > rcv_base) { //send ACK and buffer data
	    pkt.ack_flag = 1;
	    pkt.ack_n = pkt.seq_n + bytes + sizeof(struct packet_hdr);
	    send_hdr(&pkt);
	    print_send(pkt.ack_n, 0, 0, 0);
